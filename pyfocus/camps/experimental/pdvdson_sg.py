...
class eigenSolver:
    ...
    def HVecs(self, vbas):
        ...
        #self.comm.Barrier()
        wbas = torch.zeros((n, self.ndim), dtype=dtype_config.default_dtype).to(dtype_config.device)
        for i in range(n):
            wbas_s = self.HVec(vbas[i])      # 这个矩阵向量乘需要具体定义
            wbas[i] = wbas_s
        return wbas
    ...
    def solve_iter(self, v0=None, iop=4, ifplot=False):
        ...
        for niter in range(1, self.maxcycle):
            ...
            if self.iprt >= 0:
                if niter == 1:
                    print('[pdvdson]: Hc=ce with (noise,ndim,crit_e,crit_v) = (%i,%d,%.1e,%.1e)' % \
                          (self.noise, self.ndim, self.crit_e, self.crit_vec))
...
class eigenSolver_M(eigenSolver):
    def __init__(self):
        super().__init__()
        self.M_MPO_len = None
    ...
    def HVecs(self, vbas):
        ...
        wbas = torch.zeros((n, self.ndim), dtype=dtype_config.default_dtype).to(dtype_config.device)
        for i in range(n):
            wbas_s = torch.sum(torch.stack([self.HVec[k](vbas[i]) for k in range(self.M_MPO_len)]), axis = 0)      # 这个矩阵向量乘需要具体定义
            wbas[i] = wbas_s
        return wbas
...
if __name__ == '__main__':
    ...
    def test_real():
        ...
        neig = 2
        torch.random.seed(0)
        mat = torch.random.uniform(-1, 1, size=(ndim, ndim))
        mat = 0.5 * (mat + mat.T)
        efull, v = torch.linalg.eigh(mat)
    ...
    def test_complex():
        ...
        neig = 2
        torch.random.seed(0)
        mat1 = torch.random.uniform(-1, 1, size=(ndim, ndim))
        mat2 = torch.random.uniform(-1, 1, size=(ndim, ndim))
        ...
        print('Hermicity=', torch.linalg.norm(mat - mat.T.conj()))
        efull, v = torch.linalg.eigh(mat)
        ...
        solver.maxcycle = 200
        solver.ndim = ndim
        solver.diag = torch.diag(mat.real)
        solver.neig = neig
        solver.HVec = masker.matvec
        solver.noise = True  # False
        # solver.comm = MPI.COMM_WORLD
        solver.dtype = torch.complex
        # solver.mtype = MPI.C_DOUBLE_COMPLEX
        eigs, civec, nmvp = solver.solve_iter(v0=None, iop=4)
        vec = v[:, :neig].T.clone()
        ova = torch.abs(vec.matmul(civec.T.conj()))
        print('eigs  =', eigs)
        print('efull =', efull[:neig])
        print('civec =', civec.shape)
        print('vec   =', vec.shape)
