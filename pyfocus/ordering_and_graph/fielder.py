...
# Jij = (ii|jj)
def coulombMatrix(eri):
   nb = eri.shape[0]
   jij = numpy.zeros((nb,nb))
   for i in range(nb):
      for j in range(nb):
        jij[i,j] = eri[i,i,j,j]
   return jij
...
def orbitalOrderingGreedy(kij):
    forder = orbitalOrdering(kij,mode='kmat')
    forder2 = [forder[0]]
    for i in range(len(forder)):
        idx = forder2[i]
        jdxsort = numpy.argsort(-kij[idx])
        for jdx in jdxsort:
            if jdx in forder2: continue
            forder2.append(jdx)
            break
    assert len(forder2) == len(forder)
    print('fielder ordering:', list(forder))
    print('greedy ordering:', list(forder2))
    return forder2
