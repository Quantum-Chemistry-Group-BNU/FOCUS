...
void rotate_integral(const input::schedule& schd){
   ...
   umat[1].resize(norb,norb);
   umat[0].load_txt(schd.scratch+"/"+schd.urot_file);
   umat[1].load_txt(schd.scratch+"/"+schd.urot_file);
   check_orthogonality(umat[0]);

   // rotate integrals
   integral::one_body<Tm> int1e_new;
   integral::two_body<Tm> int2e_new;
   integral::rotate_spatial(int1e, int1e_new, umat);
   integral::rotate_spatial(int2e, int2e_new, umat);
