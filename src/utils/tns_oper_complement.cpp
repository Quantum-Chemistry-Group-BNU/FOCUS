...
void tns::oper_renorm_rightS(const comb& bra,
   ...
   if(ifload/2 == 0){
      ...
      oper_dot_S(k, lsupp, int2e, int1e, cqops_c, cqops_S);
   }else if(ifload/2 == 1){
   ...
   if(ifload%2 == 0){
      ...
      oper_dot_S(k0, lsupp, int2e, int1e, rqops_c, rqops_S);
   }else if(ifload%2 == 1){
   ...
   for(int i=0; i<qops.size(); i++){
	 ...
	 if(ifcal) qops[i] += oper_kernel_right_OcOr(bsite,ksite,cop_cc.T(),rop);
      }
   } // pL
   ...
   if(rtype == 0){
	 ...
	 for(const auto& cop_c : cqops_c){
	    ...
	    if(ifcal) qops[i] += oper_kernel_right_OcOr(bsite,ksite,cop_c.T(),rop);
	 }
      } // pL
   }else{
	 ...
	 for(const auto& cop_c : cqops_c){
	    ...
	    qsym rsym = qops[i].msym + cop_c.msym;
	    assert(rsym == rop.msym);
	    qops[i] += oper_kernel_right_OcOr(bsite,ksite,cop_c.T(),rop);
	 }
      }	// pL 
   } // rtype
...
void tns::oper_renorm_rightH(const comb& bra,
   ...
   if(ifload/2 == 0){
      ...
      oper_dot_S(k, rsupp, int2e, int1e, cqops_c, cqops_S);
      oper_dot_H(k, int2e, int1e, cqops_ca, cqops_H);
   }else if(ifload/2 == 1){
      // type PQ decomposition (load,build)
      assert(j == 0);
      auto pc = make_pair(i,1);
   ...
   if(ifload%2 == 0){
      ...
      rqops_ca = oper_dot_ca(k0);
      vector<int> csupp({k});
      oper_dot_S(k0, csupp, int2e, int1e, rqops_c, rqops_S);
      oper_dot_H(k0, int2e, int1e, rqops_ca, rqops_H);
   }else if(ifload%2 == 1){
      fname0r = oper_fname(scratch, p0, "rightC");
      oper_load(fname0r, rqops_c);
      if(j == 0 && i <= bra.iswitch){ 
   ...
   // 1. local term: Hc*Ir
   H += oper_kernel_right_OcIr(bsite,ksite,cqops_H[0]);
   // 2. local term: Ic*Hr
   H += oper_kernel_right_IcOr(bsite,ksite,rqops_H[0]);
   // 3. pC^+ S_pC^R + h.c. 
   map<int,int> Sr_pL2pos;
   for(int idx=0; idx<rqops_S.size(); idx++){
      int pL = rqops_S[idx].index[0];
   ...
   for(const auto& cop_c : cqops_c){
      ...
      qsym rsym = H.msym - cop_c.msym;
      assert(rsym == rop.msym);
      H += oper_kernel_right_OcOr(bsite,ksite,cop_c,rop);
      H += oper_kernel_right_OrOc(bsite,ksite,rop.T(),cop_c.T());
   }
   // 4. qR^+ S_qR^C + h.c. 
   map<int,int> Sc_pL2pos;
   for(int idx=0; idx<cqops_S.size(); idx++){
   ...
   for(const auto& rop_c : rqops_c){
      ...
      qsym csym = H.msym - rop_c.msym;
      assert(csym == cop.msym);
      H += oper_kernel_right_OrOc(bsite,ksite,rop_c,cop);
      H += oper_kernel_right_OcOr(bsite,ksite,cop.T(),rop_c.T());
   }
   ...
   if(rtype == 0){
	 ...
	 int qC = cop_cc.index[1];
         ...
         if(ifcal){
	    H += oper_kernel_right_OcOr(bsite,ksite,cop_cc,rop);
	    H += oper_kernel_right_OrOc(bsite,ksite,rop.T(),cop_cc.T());
	 }
      }
	 ...
	 int sC = cop_ca.index[1];
         ...
         if(ifcal) H += oper_kernel_right_OcOr(bsite,ksite,cop_ca,rop);
      }
   }else{
      // type-PQ decomposition
	 ...
	 qsym rsym = H.msym - cop_ca.msym;
	 assert(rsym == rop.msym);
	 H += oper_kernel_right_OcOr(bsite,ksite,cop_ca,rop);
	 H += oper_kernel_right_OrOc(bsite,ksite,rop.T(),cop_ca.T());
      }
   } // rtype
   qops.push_back(H);
