...
qtensor2::qtensor2(const qsym& sym1,
   ...
   for(const auto& pr : qrow){
	 ...
	 // symmetry rule for <r|O|c>
         if(ifconserve(qr,qc)){
	    qblocks[key] = matrix<double>(rdim,cdim);
	 }else{
	    qblocks[key] = matrix<double>();
	 }
      }
   }
}
...
matrix<double> qtensor2::to_matrix() const{
   int m = get_dim_row();
   int n = get_dim_col();
...
qtensor2 qtensor2::T() const{
   ...
   for(const auto& p : qblocks){
      ...
      if(blk.size() > 0){
         ...
         qt2.qblocks[tkey] = matrix<double>();
      } 
   }
   return qt2;
}
	 ...
	 if(blk.size() > 0){
	    if(qr != qc){
	       ...
	       exit(1);
            ...
            int ndim = pr.second;
            double diff = linalg::normF(blk - identity_matrix<double>(ndim));
...
qtensor2 qtensor2::get_rdm_row() const{
   ...
   for(const auto& pr : qrow){
	 ...
	 rdm.qblocks[key] += xgemm("N","N",blk,blk.T());
      }
   }
   return rdm;
}
...
qtensor2 qtensor2::get_rdm_col() const{
   ...
   for(const auto& pc : qcol){
	 ...
	 const auto& blk = qblocks.at(make_pair(qr,qc));
	 if(blk.size() == 0) continue;
	 rdm.qblocks[key] += xgemm("N","N",blk.T(),blk);
      }
   }
   return rdm;
}
