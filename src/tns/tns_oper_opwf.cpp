...
qtensor3 tns::oper_kernel_Pwf(const string& superblock,
   ...
   qsym sym_opwf = ifdagger? -symP+ksite.sym : symP+ksite.sym;  
   qtensor3 opwf(sym_opwf, ksite.qmid, ksite.qrow, ksite.qcol, ksite.dir);
   // 
   ...
   if(ifP1){
      opwf += oper_kernel_OIwf(superblock,ksite,qops1['P'].at(index),ifdagger);
   }else{
      ...
      for(const auto& op1A : qops1['A']){
         ...
         int s1 = sr.first;
         int r1 = sr.second;
         op1Pdag += int2e.get(p,q,s1,r1)*op1A.second;
      }
   ...
   if(ifP2){
      opwf += oper_kernel_IOwf(superblock,ksite,qops2['P'].at(index),0,ifdagger);
   }else{
      ...
      for(const auto& op2A : qops2['A']){
         ...
         int s2 = sr.first; 
         int r2 = sr.second;
         op2Pdag += int2e.get(p,q,s2,r2)*op2A.second;
      }
   ...
   if(qops1['C'].size() < qops2['C'].size()){
	 ...
	 auto& it = qops2['C'].begin()->second;
         ...
         for(const auto& op2C : qops2['C']){
            ...
            int r2 = op2C.first;
            op2 -= int2e.get(p,q,s1,r2)*op2C.second;
         }
	 if(ifdagger){
	 ...
	 auto& it = qops1['C'].begin()->second;
         ...
         for(const auto& op1C : qops1['C']){
            ...
            int s1 = op1C.first;
            op1 -= int2e.get(p,q,s1,r2)*op1C.second;
         }
	 if(ifdagger){
...
qtensor3 tns::oper_kernel_Qwf(const string& superblock,
   ...
   if(ifQ1){
      ...
      for(const auto& op1B : qops1['B']){
         ...
         int q1 = qr.first;
         int r1 = qr.second;
         op1Q += int2e.get(p,q1,s,r1)*op1B.second;
      }
   ...
   if(ifQ2){
      ...
      for(const auto& op2B : qops2['B']){
         ...
         int q2 = qr.first;
         int r2 = qr.second;
         op2Q += int2e.get(p,q2,s,r2)*op2B.second;
      }
   ...
   if(qops1['C'].size() < qops2['C'].size()){
	 ...
	 for(const auto& op2C : qops2['C']){
	    if(op2.sym != op2C.second.sym) continue;
	    int r2 = op2C.first;
	    op2 += int2e.get(p,q1,s,r2)*op2C.second;
	 }
	 ...
	 for(const auto& op2C : qops2['C']){
            if(op2.sym != op2C.second.sym) continue;
	    int q2 = op2C.first;
	    op2 -= int2e.get(p,q2,s,r1)*op2C.second;
	 }
	 ...
	 for(const auto& op1C : qops1['C']){
	    if(op1.sym != op1C.second.sym) continue;
	    int q1 = op1C.first;
	    op1 += int2e.get(p,q1,s,r2)*op1C.second;
	 }
	 opwf += oper_kernel_OOwf(superblock,ksite,op1,op2.T(),1);
      }
      // 4. (-<pq2||sr1> r1) * q2^+
	 ...
	 for(const auto& op1C : qops1['C']){
            if(op1.sym != op1C.second.sym) continue;
	    int r1 = op1C.first;
	    op1 -= int2e.get(p,q2,s,r1)*op1C.second;
	 }
	 opwf += oper_kernel_OOwf(superblock,ksite,op1.T(),op2,1);
      }
   }
...
qtensor3 tns::oper_kernel_Swf(const string& superblock,
   ...
   qsym sym_opwf = ifdagger? -symS+ksite.sym : symS+ksite.sym;  
   qtensor3 opwf(sym_opwf, ksite.qmid, ksite.qrow, ksite.qcol, ksite.dir);
   //
   ...
   if(ifP1){
      ...
      if(qops1['A'].size() < qops2['C'].size()){
	    ...
	    auto& it = qops2['C'].begin()->second;
            ...
            for(const auto& op2C : qops2['C']){
               if(op2.sym != op2C.second.sym) continue;
               int q2 = op2C.first;
               op2 += int2e.get(p,q2,s1,r1)*op2C.second;
            }
	    if(ifdagger){
               opwf += oper_kernel_OOwf(superblock,ksite,op1,op2.T(),1);
	    }else{
	 ...
	 for(const auto& op2C : qops2['C']){
	    ...
	    for(const auto& op1A : qops1['A']){
	       ...
	       int s1 = sr.first;
	       int r1 = sr.second;
	       op1 += int2e.get(p,q2,s1,r1)*op1A.second;
	    }
	    if(ifdagger){
	       opwf += oper_kernel_OOwf(superblock,ksite,op1,op2.T(),1);
	    }else{
   ...
   if(ifQ1){
      ...
      if(qops1['B'].size() < qops2['C'].size()){
	    ...
	    qtensor2 op2(-rsym, it.qrow, it.qcol);
            for(const auto& op2C : qops2['C']){
               if(op2.sym != op2C.second.sym) continue;
               int r2 = op2C.first;
               op2 -= int2e.get(p,q1,s1,r2)*op2C.second;
            }
            if(ifdagger){
	       opwf += oper_kernel_OOwf(superblock,ksite,op1.T(),op2,1);
	    }else{
	 ...
	 for(const auto& op2C : qops2['C']){
	    ...
	    for(const auto& op1B : qops1['B']){
	       ...
	       int q1 = qs.first;
	       int s1 = qs.second;
	       op1 -= int2e.get(p,q1,s1,r2)*op1B.second;
	    }
	    if(ifdagger){
	       opwf += oper_kernel_OOwf(superblock,ksite,op1.T(),op2,1);
	    }else{
      ...
      if(qops1['C'].size() < qops2['A'].size()){
	    ...
	    auto& it = qops2['A'].begin()->second;
            ...
            for(const auto& op2A : qops2['A']){
               ...
               int s2 = sr.first;
               int r2 = sr.second;
               op2 += int2e.get(p,q1,s2,r2)*op2A.second;
            }
            if(ifdagger){
	    ...
	    for(const auto& op1C : qops1['C']){
 	       if(op1.sym != op1C.second.sym) continue;
	       int q1 = op1C.first;
	       op1 += int2e.get(p,q1,s2,r2)*op1C.second;
	    }
	    if(ifdagger){
	       opwf += oper_kernel_OOwf(superblock,ksite,op1.T(),op2,0);
	    }else{
      ...
      if(qops1['C'].size() < qops2['B'].size()){
	    ...
	    auto& it = qops2['B'].begin()->second;
            ...
            for(const auto& op2B : qops2['B']){
               ...
               int q2 = qr.first;
               int r2 = qr.second;
               op2 += int2e.get(p,q2,s1,r2)*op2B.second;
            }
	    if(ifdagger){
               opwf += oper_kernel_OOwf(superblock,ksite,op1,op2.T(),0);
	    }else{
	    ...
	    for(const auto& op1C : qops1['C']){
	       if(op1.sym != op1C.second.sym) continue;
	       int s1 = op1C.first;
	       op1 += int2e.get(p,q2,s1,r2)*op1C.second;
	    }
	    if(ifdagger){
	       opwf += oper_kernel_OOwf(superblock,ksite,op1,op2.T(),0);
	    }else{
   ...
   if(ifA1P2){ 
      ...
      if(qops1['A'].size() < qops2['A'].size()){
	    ...
	    auto& it = qops2['A'].begin()->second;
            ...
            for(const auto& op2A : qops2['A']){
               ...
               int s2 = sr.first;
               int r2 = sr.second;
               op2 += int2e.get(p1,q1,s2,r2)*op2A.second;
            } // rs
            opwf += oper_kernel_OOwf(superblock,ksite,op1,op2.T(),0);
            opwf += oper_kernel_OOwf(superblock,ksite,op1.T(),op2,0);
         } // pq
	 ...
	 for(const auto& op2A : qops2['A']){
	    ...
	    for(const auto& op1A : qops1['A']){
	       ...
	       int p1 = pq.first;
	       int q1 = pq.second;
	       op1 += int2e.get(p1,q1,s2,r2)*op1A.second;
	    }
	    opwf += oper_kernel_OOwf(superblock,ksite,op1,op2.T(),0);
	    opwf += oper_kernel_OOwf(superblock,ksite,op1.T(),op2,0);
	 }
   ...
   if(ifB1Q2){
      ...
      if(qops1['B'].size() < qops2['B'].size()){
         ...
         for(const auto& op1B : qops1['B']){
            ...
            for(const auto& op2B : qops2['B']){
               ...
               int q2 = qr.first;
               int r2 = qr.second;
               op2 += int2e.get(p1,q2,s1,r2)*op2B.second;
            }
            opwf += oper_kernel_OOwf(superblock,ksite,op1,op2,0);
	 }
      }else{
	 ...
	 for(const auto& op2B : qops2['B']){
	    ...
	    for(const auto& op1B : qops1['B']){
	       ...
	       int p1 = ps.first;
	       int s1 = ps.second;
	       op1 += int2e.get(p1,q2,s1,r2)*op1B.second;
	    }
	    opwf += oper_kernel_OOwf(superblock,ksite,op1,op2,0);
	 }
      }
