...
renorm_basis tns::get_rbasis_phys(){
   ...
   for(int i=0; i<4; i++){
      ...
      rbasis[i].coeff = linalg::identity_matrix<double>(1);
   }
   return rbasis;
}
...
pair<int,double> product_space::projection(const vector<vector<double>>& vs,
   ...
   for(auto it = qsecA.cbegin(); it != qsecA.cend(); ++it){
	 ...
	 for(int i=0; i<dimAs; i++){
      ...
      matrix<double> rhol(dimAs,dimAs);
      int nroots = vs.size();
      for(int iroot = 0; iroot<nroots; iroot++){
         // vlr for sym sector
         matrix<double> vlr(dimAs,dimB);
         ...
         rhol += xgemm("N","N",vlr,vlr.T());
      }
      rhol *= 1.0/nroots;
      vector<double> eig(dimAs);
      matrix<double> rho_tmp(rhol);
      eig_solver(rho_tmp,eig,rhol,1);
      // compute entropy
      int dimAi = 0;
      double sumi = 0.0;
      for(int i=0; i<dimAs; i++){ 
...
renorm_basis product_space::right_projection(const vector<vector<double>>& vs,
   ...
   for(auto it = qsecB.cbegin(); it != qsecB.cend(); ++it){
	 ...
	 u.resize(dimBs,dimBs);
	    ...
	    u += xgemm("N","N",vrl,vrl.T()); 
         }
         u *= 1.0/nroots;
         matrix<double> u_tmp(u);
	 eig_solver(u_tmp,sig,u,1);
      } 
