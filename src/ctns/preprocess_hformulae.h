...
namespace ctns{
   ...
   template <typename Tm, typename QTm>
      void preprocess_formulae_Hxlist2Direct(const oper_dictmap<Tm>& qops_dict,
            const std::map<std::string,int>& oploc,
            const symbolic_task<Tm>& H_formulae,
            const QTm& wf,
            Hxlist2<Tm>& Hxlst2,
         ...
         Hxlst2.resize(nnzblk);
         for(int it=0; it<hsize; it++){
            Hmu_vec[it].gen_Hxlist2Direct(wf.info, Hxlst2, blksize, blksize0, cost, false);
            Hmu_vec[it].gen_Hxlist2Direct(wf.info, Hxlst2, blksize, blksize0, cost, true);
         }
         auto tb = tools::get_time();
         ...
         if(debug){
            ...
            std::cout << "T(Hmu/Hxlist/tot)="
               ...
               << tools::get_duration(t1-t0) 
               << std::endl;
            tools::timing("preprocess_formulae_Hxlist2Direct", t0, t1);
         }
      }
