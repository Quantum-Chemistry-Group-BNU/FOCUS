#ifndef CTNS_COMB_ALG_H
#define CTNS_COMB_ALG_H
...
std::vector<Tm> rcanon_CIcoeff(const comb<Tm>& icomb,
   ...
   for(int i=icomb.topo.nbackbone-1; i>=0; i--){
	 ...
	 auto mdx = get_mdx_phys<Tm>(state, node.pindex);
	 auto qt2 = site.fix_mid(mdx);
	 ...
	 // propogate symmetry from leaves down to backbone
	    ...
	    auto mdx = get_mdx_phys<Tm>(state, nodej.pindex);
	    auto qt2 = site.fix_mid(mdx); 
	    if(j==nodes[i].size()-1){
	       qt2_u = qt2;
	    }else{
...
template <typename Tm>
linalg::matrix<Tm> rcanon_CIovlp(const comb<Tm>& icomb,
...
// <CTNS[i]|CTNS[j]> 
template <typename Tm>
linalg::matrix<Tm> get_Smat(const comb<Tm>& icomb){ 
