...
std::vector<double> onedot_Hdiag(oper_dict<Tm>& cqops,
			         oper_dict<Tm>& lqops,
			         oper_dict<Tm>& rqops,
   ...
   for(int bm=0; bm<wf.mids(); bm++){
      for(int br=0; br<wf.rows(); br++){
	    ...
	    auto& rblk = Hr(bc,bc); // row->col
            for(int m=0; m<wf.qmid.get_dim(bm); m++){
               for(int c=0; c<wf.qcol.get_dim(bc); c++){
                  for(int r=0; r<wf.qrow.get_dim(br); r++){
...
/*
   // 2. density-density interactions: B*Q terms 
