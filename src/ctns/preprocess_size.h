...
namespace ctns{
      ...
      if(isym == 0){
         ...
         for(int nop=0; nop<=nbody; nop++){
            for(int n=-nop; n<=nop; n+=2){
               for(int tm=-nop; tm<=nop; tm+=2){
                  sym_ops.insert( qsym(2,n,tm) );
               }
            }
         }
      }else if(isym == 3){
         ...
         for(int nop=0; nop<=nbody; nop++){
            for(int n=-nop; n<=nop; n+=2){
               for(int ts=nop; ts>=0; ts-=2){
                  sym_ops.insert( qsym(3,n,ts) );
               }
            }
         }
      }else{
         std::cout << "error: no such option for get_qsym_allops! isym=" << isym << std::endl;
         exit(1);
      }
      return sym_ops;
   }
   ...
   template <bool ifab, typename Tm>
      size_t preprocess_opsize(const qoper_dictmap<ifab,Tm>& qops_dict){
         size_t opsize = 0;
   ...
   template <bool ifab, typename Tm>
      size_t preprocess_wfsize(const qinfo3type<ifab,Tm>& wf3info, 
         ...
         for(const auto& sym_op : sym_ops){
            ...
            info.init(sym, qrow, qcol, qmid, dir); // by default: CRcouple
            info_dict[sym] = info;
   ...
   template <bool ifab, typename Tm>
      size_t preprocess_wfsize(const qinfo4type<ifab,Tm>& wf4info, 
            std::map<qsym,qinfo4type<ifab,Tm>>& info_dict,
            const int nbody=2){
