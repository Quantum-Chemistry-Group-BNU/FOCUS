...
namespace ctns{
   ...
   // 
   // oper_pool with std::map 
   //

   template <bool ifab, typename Tm>
      using qoperData_pool = std::map<std::string,qoper_dict<ifab,Tm>>;
   ...
   template <bool ifab, typename Tm>
      struct qoper_pool{
         public:
            // constuctor
            qoper_pool(const int _iomode, const bool _debug): iomode(_iomode), debug(_debug){}
            // check whether frop exist in the pool
            bool exist(const std::string& frop) const{
               return qstore.find(frop) != qstore.end();
            }
            // access
            const qoper_dict<ifab,Tm>& at(const std::string& fqop) const{
               assert(this->exist(fqop));
               return qstore.at(fqop);
            }
            qoper_dict<ifab,Tm>& operator[](const std::string& fqop){
               return qstore[fqop];
            }
            ...
            void clear_from_cpumem(const std::vector<std::string> fneed,
                  ...
                  const std::string frop_beingsaved=""); 
   ...
   template <bool ifab, typename Tm>
      void qoper_pool<ifab,Tm>::fetch_to_memory(const std::vector<std::string> fneed, const bool ifgpu){
         auto t0 = tools::get_time();
         if(debug){
            std::cout << "ctns::qoper_pool::fetch_to_memory: ifgpu=" << ifgpu
         ...
         if(debug){
            ...
            std::cout << "----- TIMING FOR qoper_pool::fetch_to_memory : "
   ...
   template <bool ifab, typename Tm>
      void oper_fetch(qoperData_pool<ifab,Tm>& qstore,
   ...
   template <bool ifab, typename Tm>
      void qoper_pool<ifab,Tm>::fetch_to_cpumem(const std::vector<std::string> fneed, const bool async_fetch){
         auto t0 = tools::get_time();
         if(debug){
            std::cout << "ctns::qoper_pool::fetch_to_cpumem: async_fetch=" << async_fetch
               << " size=" << fneed.size() << std::endl;
            this->display("in");
         }
         ...
         if(debug){
            this->display("out");
            auto t1 = tools::get_time();
            std::cout << "----- TIMING FOR qoper_pool::fetch_to_cpumem : "
               << tools::get_duration(t1-t0) << " S -----"
               << std::endl;
         }
      }
   ...
   template <bool ifab, typename Tm>
      void qoper_pool<ifab,Tm>::join_and_erase(const std::vector<std::string> frelease,
         ...
         auto t0 = tools::get_time();
         if(debug){
            std::cout << "ctns::qoper_pool::join_and_erase : size=" << frelease.size() << std::endl; 
            ...
            {
               bool ifexist = this->exist(frop_prev);
               auto result = std::find(fneed_next.begin(), fneed_next.end(), frop_prev);
               bool iferase = (result == fneed_next.end());
               std::cout << " erase: frop_prev=" << frop_prev << " ifexist=" << ifexist << " iferase=" << iferase << std::endl;
            }
            this->display("in");
         }

         /// in fact, only fetch and remove will be joined during sweep
         this->join_all();
         auto t1 = tools::get_time();
   ...
   template <bool ifab, typename Tm>
      void qoper_pool<ifab,Tm>::clear_from_memory(const std::vector<std::string> fclear,
            const std::vector<std::string> fneed_next){
         auto t0 = tools::get_time();
         if(debug){
            std::cout << "ctns::qoper_pool::clear_from_memory: size=" << fclear.size() << std::endl; 
            ...
            this->display("in");
            get_mem_status(0, 0, "before clear", true);
         }
         for(const auto& fqop : fclear){
         ...
         if(debug){
            this->display("out");
            get_mem_status(0, 0, "after clear", true);
            auto t1 = tools::get_time();
   ...
   template <bool ifab, typename Tm>
      void qoper_pool<ifab,Tm>::clear_from_cpumem(const std::vector<std::string> fclear,
            const std::vector<std::string> fneed_next,
            const bool ifkeepcoper,
            const std::string frop_beingsaved){
         auto t0 = tools::get_time();
         if(debug){
            std::cout << "ctns::qoper_pool::clear_from_cpumem: size=" << fclear.size() 
               << " frop_beingsaved=" << frop_beingsaved
               << std::endl; 
            for(const auto& fqop : fclear){
               bool ifexist = this->exist(fqop);
               auto result = std::find(fneed_next.begin(), fneed_next.end(), fqop); 
               bool ifclear = (result == fneed_next.end()) && (fqop != frop_beingsaved); 
               std::cout << " clear: fqop=" << fqop << " ifexist=" << ifexist << " ifclear=" << ifclear << std::endl;
            }
            this->display("in");
            get_mem_status(0, 0, "before clear cpumem", true);
         }
         for(int i=0; i<fclear.size(); i++){
            if(ifkeepcoper && i >= 2) continue; // skip op[c2/c1]
            const auto& fqop = fclear[i];
            if(fqop == frop_beingsaved) continue; // DO NOT remove CPU space, since saving may not finish!
            auto result = std::find(fneed_next.begin(), fneed_next.end(), fqop);
            if(result != fneed_next.end()) continue;
            qstore[fqop].clear();
         }
         ...
         if(debug){
            this->display("out");
            get_mem_status(0, 0, "after clear cpumem", true);
            auto t1 = tools::get_time();
   ...
   template <bool ifab, typename Tm>
      void qoper_pool<ifab,Tm>::save_to_disk(const std::string frop, 
         ...
         if(debug){
            auto t1 = tools::get_time();
            std::cout << "----- TIMING FOR qoper_pool::save_to_disk : "
               << tools::get_duration(t1-t0) << " S -----"
               << std::endl;
         }
      }
