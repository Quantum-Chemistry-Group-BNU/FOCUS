...
namespace ctns{
   ...
   template <typename Tm, typename QTm>
      void preprocess_formulae_Rlist2(const oper_dictmap<Tm>& qops_dict,
            const std::map<std::string,int>& oploc,
            const renorm_tasks<Tm>& rtasks,
            const QTm& wf,
            const rintermediates<Tm>& rinter,
...
/*
         ...
         int rsize = rtasks.size();
         std::vector<Hmu_ptr<Tm>> Hmu_vec(rsize);
         for(int it=0; it<rsize; it++){
            Hmu_vec[it].init(it, rtasks, qops_dict, rinter, oploc); 
         } // it
         auto ta = tools::get_time();
         ...
         Rlst2.resize(nnzblk);
         for(int it=0; it<rsize; it++){
            Hmu_vec[it].gen_Hxlist2(wf.info, Rlst2, blksize, cost, false);
            Hmu_vec[it].gen_Hxlist2(wf.info, Rlst2, blksize, cost, true);
         }
         auto tb = tools::get_time();
