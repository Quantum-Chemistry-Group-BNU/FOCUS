...
public:
   void init(const int it,
	     ...
	     const std::map<std::string,int>& posmap,
	     Tm** qops_addr);
...
public:
   ...
   Tm* addr[4] = {nullptr,nullptr,nullptr,nullptr};
   ...
   for(int i=0; i<wf_info._nnzaddr.size(); i++){
	 ...
	 Hxblk.dagger[k] = dagger[k]^ifdagger;
	    ...
	    if(bi[k] == -1){
	       ...
	       Hxblk.addr[k] = addr[k]+(info[k]->_offset[jdx]-1);
	    }
         }
      }
      ...
      Hxblk.gen_MMlist(3);
      tmpsize = std::max(tmpsize, Hxblk.tmpsize);
      cost += Hxblk.cost;
