...
// combine support
std::vector<int> oper_combine_cindex(const std::vector<int>& cindex1,
...
void oper_combine_opC(const std::vector<int>& cindex1,
		      const std::vector<int>& cindex2,
   ...
   iop = 1;
   for(int p1 : cindex1){
      info.append(std::make_pair(iop,p1));
   }
   // 2. I1*p2^+ 
   iop = 2;
   for(int p2 : cindex2){
      info.append(std::make_pair(iop,p2));
   }
}
...
void oper_combine_opA(const std::vector<int>& cindex1,
		      const std::vector<int>& cindex2,
		      const bool& ifkr,
		      std::vector<std::pair<int,int>>& info){
   int iop; 
