...
void opt_sweep_onedot(const input::schedule& schd,
   ...
   std::cout << "tns::opt_sweep_onedot" << std::endl;
   ...
   bool cturn = (icomb.topo.node_type(p0) == 3 && p1.second == 1);
...
void opt_sweep_summary(const input::schedule& schd,
		       ...
		       // current sweep
		       const std::vector<dot_result>& sweep_result, 
		       // updated data
		       const int isweep,
		       const std::vector<double>& timing,
		       std::vector<dot_result>& sweep_summary){
   //
   ...
   sweep_summary[isweep] = sweep_result[minpos];
   ...
   std::cout << "results: dwt, energies (delta_e)" << std::endl;
   const auto& eopt = sweep_summary[isweep].eopt;
   for(int jsweep=0; jsweep<=isweep; jsweep++){
      const auto& dwt = sweep_summary[jsweep].dwt;
      const auto& eopt0 = sweep_summary[jsweep].eopt;
