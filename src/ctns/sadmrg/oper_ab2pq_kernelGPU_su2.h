...
namespace ctns{
   ...
   template <typename Qm, typename Tm, std::enable_if_t<!Qm::ifabelian,int> = 0>
      void oper_a2pGPU(const comb<Qm,Tm>& icomb,
         ...
         if(maxsize == 0) return;
         if(rank == 0){
            std::cout << "ctns::oper_a2pGPU maxsize=" << maxsize << ":" << tools::sizeGB<Tm>(maxsize) << "GB" << std::endl;
         }
   ...
   template <typename Qm, typename Tm, std::enable_if_t<!Qm::ifabelian,int> = 0>
      void oper_b2qGPU(const comb<Qm,Tm>& icomb,
         ...
         if(maxsize == 0) return;
         if(rank == 0){
            std::cout << "ctns::oper_b2qGPU maxsize=" << maxsize << ":" << tools::sizeGB<Tm>(maxsize) << "GB" << std::endl;
         }
         maxsize = maxsize*sizeof(Tm);
         Tm* dev_work = (Tm*)GPUmem.allocate(maxsize);
