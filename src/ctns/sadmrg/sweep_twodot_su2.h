#ifndef SWEEP_TWODOT_SU2_H
#define SWEEP_TWODOT_SU2_H
...
namespace ctns{
   ...
   template <typename Qm, typename Tm, std::enable_if_t<!Qm::ifabelian,int> = 0>
      void sweep_twodot(comb<Qm,Tm>& icomb,
         ...
         if(debug){
            std::cout << "ctns::sweep_twodot(su2)"
               << " alg_hvec=" << alg_hvec
         ...
         stensor4su2<Tm> wf(sym_state, ql, qr, qc1, qc2); // LC1andC2Rcouple
         size_t ndim = wf.size();
...
*/
         ...
         // 3.2 prepare HVec
         std::map<qsym,qinfo4su2<Tm>> info_dict;
         size_t worktot=0;
         std::string fname;
