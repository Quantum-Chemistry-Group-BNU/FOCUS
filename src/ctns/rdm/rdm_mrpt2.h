   ...
   template <typename Qm, typename Tm>
      double rdm_mrpt2(const comb<Qm,Tm>& icomb,
         ...
         if(schd.ctns.alg_rdm == 0){
            tools::exit("error: only support alg_rdm != 0 in rdm_mprt2");
         }
...
#ifndef SERIAL
            if(size > 1){
               mpi_wrapper::allreduce(icomb.world, &ept2v, 1);
               mpi_wrapper::allreduce(icomb.world, &ept2c, 1);
            }
#endif
         ...
         ept2 = ept2v + ept2c;
         if(debug){
            std::cout << std::fixed << std::setprecision(schd.ctns.outprec);
            std::cout << "\nept2v = " << ept2v << std::endl; 
            std::cout << "ept2c = " << ept2c << std::endl; 
            std::cout << "ept2  = " << ept2 << std::endl; 
