...
namespace ctns{
   ...
   template <typename Tm>
      struct rintermediates{
         ...
         public:
            std::map<std::pair<int,int>,size_t> _offset; // map from (it,idx) in rtasks to offset
...
/*
         ...
         for(int it=0; it<rtasks.size(); it++){
            const auto& HTerm = rtasks.tasks[it];
...
#pragma omp parallel for schedule(dynamic)
         ...
         for(size_t idx=0; idx<_count; idx++){
            ...
            int i = item.first;
            int j = item.second;
            const auto& sop = rtasks.tasks[i].terms[j];
