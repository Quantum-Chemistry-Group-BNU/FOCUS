...
namespace fci{
   ...
   // construct Hij	   
   template <typename Tm>
      void sparse_hamiltonian<Tm>::get_hamiltonian(const fock::onspace& space,
            const integral::two_body<Tm>& int2e,
            const integral::one_body<Tm>& int1e,
            const double ecore,
            const bool Htype,
   ...
   // construct Hij	   
   template <typename Tm>
      void sparse_hamiltonian<Tm>::make_hamiltonian(const fock::onspace& space,
            const product_space& pspace,
            const coupling_table& ctabA,
            const coupling_table& ctabB,
            const integral::two_body<Tm>& int2e,
   ...
   // 	 	       {I_B,J_B} differ by zero (I_B=J_B)
   template <typename Tm>
      void sparse_hamiltonian<Tm>::get_HIJ_A1122_B00(const fock::onspace& space,
            const product_space& pspace,
            const coupling_table& ctabA,
            const coupling_table& ctabB,
            const integral::two_body<Tm>& int2e,
   ...
   // 		       {I_B,J_B} differ by single/double
   template <typename Tm>
      void sparse_hamiltonian<Tm>::get_HIJ_A00_B1122(const fock::onspace& space,
            const product_space& pspace,
            const coupling_table& ctabA,
            const coupling_table& ctabB,
            const integral::two_body<Tm>& int2e,
   ...
   // 		       {I_B,J_B} differ by single
   template <typename Tm>
      void sparse_hamiltonian<Tm>::get_HIJ_A11_B11(const fock::onspace& space,
            const product_space& pspace,
            const coupling_table& ctabA,
            const coupling_table& ctabB,
            const integral::two_body<Tm>& int2e,
   ...
   // 		     + C21_A*C01_B
   template <typename Tm>
      void sparse_hamiltonian<Tm>::get_HIJ_ABmixed1(const fock::onspace& space,
            const product_space& pspace,
            const coupling_table& ctabA,
            const coupling_table& ctabB,
            const integral::two_body<Tm>& int2e,
   ...
   // <I_A,I_B|H|J_A,J_B> = C02_A*C20_B + C20_A*C02_B
   template <typename Tm>
      void sparse_hamiltonian<Tm>::get_HIJ_ABmixed2(const fock::onspace& space,
            const product_space& pspace,
            const coupling_table& ctabA,
            const coupling_table& ctabB,
            const integral::two_body<Tm>& int2e,
   ...
   // compare with full construction
   template <typename Tm>
      void sparse_hamiltonian<Tm>::check(const fock::onspace& space,
            const integral::two_body<Tm>& int2e,
            const integral::one_body<Tm>& int1e,
            const double ecore,
            const double thresh){
   ...
   // analyze the magnitude of Hij
   template <typename Tm>
      void sparse_hamiltonian<Tm>::analysis(const double thresh){
   ...
   template <typename Tm>
      ...
      //            const double ecore) const{
         ...
         indptr[0] = 0;
         for(int i=0; i<dim; i++){
            indptr[i+1] = indptr[i] + connect[i].size() + 1; 
         }
         int nnz = indptr[dim];
         ...
         */
         ...
         for(int i=0; i<dim; i++){
            ...
            int sta = indptr[i];
            int end = indptr[i+1]-1;
            std::copy_n(connect[i].data(), innz, &indices[sta]);
            indices[end] = i;
         ...
         ofs.write((char*)(indptr.data()), sizeof(int)*(dim+1));
         ofs.write((char*)(indices.data()), sizeof(int)*nnz);
         ofs.write((char*)(data.data()), sizeof(Tm)*nnz);
         ofs.close();
      }
